---
name: Ninja
real_name: Tyler Blevins
born: '1991-06-05'
nationality: American
primary_platform: Twitch
platforms: ['platforms:twitch', 'platforms:youtube-live']
genres: ['Fortnite', 'Variety']
started: '2011'
followers: 19000000
active: true
verified: true
handles:
  Twitch: https://www.twitch.tv/ninja
  YouTube: https://www.youtube.com/@Ninja
  X: https://x.com/Ninja
related: ['platforms:twitch', 'events:mixer-shutdown-2020', 'events:ninja-drake-fortnite-2018']
influenced: ['streamers:tfue', 'streamers:myth', 'streamers:clix']
successors: ['streamers:xqc', 'streamers:kai-cenat']
tags: ['fortnite', 'american', 'english', 'gaming']
description: American streamer who was the first gaming creator to reach mainstream household-name status, driven by his Fortnite dominance during the game's 2017–2018 peak.
sources:
  - https://en.wikipedia.org/wiki/Ninja_(streamer)
updated: '2026-04-19'
records:
  - label: Twitch concurrent viewers (then-record)
    value: ~628K
    context: Fortnite with Drake
    date: '2018-03'
  - label: Twitch followers (peak)
    value: 19M+
  - label: Time spent as Fortnite streaming face
    value: 2017–2020
timeline:
  - date: '2011'
    label: Begins streaming competitive Halo
    kind: start
  - date: '2017'
    label: Switches full-time to Fortnite on Twitch
    kind: platform
  - date: '2018-03'
    label: Fortnite stream with Drake hits ~628K concurrent
    kind: record
    ref: events:ninja-drake-fortnite-2018
  - date: '2019-08'
    label: Signs exclusive Mixer deal
    kind: deal
  - date: '2020-06'
    label: Mixer shuts down; contract paid out
    kind: event
    ref: events:mixer-shutdown-2020
  - date: '2020-09'
    label: Returns to Twitch
    kind: platform
  - date: '2024-03'
    label: Publicly discloses melanoma diagnosis
    kind: event
quotes:
  - text: "The streamer thing wasn't a career until it was. Then it was everything."
    source: ESPN interview
    date: '2018'
---

## Career

Tyler "Ninja" Blevins began his career as a competitive Halo player before transitioning to streaming Fortnite on Twitch in 2017. His March 2018 Fortnite stream with rapper Drake briefly set the Twitch concurrent-viewership record (~628K) and pushed him into mainstream US culture — late-night TV appearances, an ESPN magazine cover, a Red Bull deal.

In August 2019 he signed an exclusive deal with Microsoft's Mixer platform. When Mixer shut down in July 2020, Microsoft released him and paid out the remainder of his contract. He briefly streamed on YouTube before returning to Twitch in September 2020.

## Cultural impact

Ninja is the most prominent example of a platform-spanning creator whose deal structure ended with the platform's closure — a cautionary tale that still shapes how top creators negotiate platform exclusivity.

## Health

In March 2024, Ninja publicly disclosed a melanoma diagnosis and underwent treatment; he returned to streaming in the months that followed.
